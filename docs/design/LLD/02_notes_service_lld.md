...
| :--- | :--- | :--- |
| note_id | UUID | Unique ID (PK) |
| user_id | UUID | Owner ID (FK) |
...
- Pattern: r"\\\[\\\[(.\*?)\\\]\\\]".
- Extracts titles within double brackets to create relationships in the note_links table.
...
| note_link_created | Triggered when a new \[\[link\]\] is detected. |
