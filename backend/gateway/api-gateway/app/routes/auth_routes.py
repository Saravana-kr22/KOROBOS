...
Auth route proxy — forwards /api/v1/auth/* to the auth-service.
"""
...
logger = logging.getLogger("api-gateway.routes.auth")
...
async def proxy_auth(request: Request, path: str):
    """Forward all /api/v1/auth/* requests to the auth-service."""
    registry = ServiceRegistry()
    target_base = registry.get_service_url("auth")

    if not target_base:
        return JSONResponse(
            status_code=503,
            content={"status": "error", "error": {"code": "SERVICE_UNAVAILABLE", "message": "Auth service not configured"}},
        )
