...
Graph route proxy — forwards /api/v1/graph/* to the graph-service.
"""
...
logger = logging.getLogger("api-gateway.routes.graph")


@router.api_route(
    "/{path:path}",
    methods=["GET", "POST", "PUT", "DELETE", "PATCH"],
    operation_id="proxy_graph",
)
async def proxy_graph(request: Request, path: str):
    """Forward all /api/v1/graph/* requests to the graph-service."""
    registry = ServiceRegistry()
    target_base = registry.get_service_url("graph")
    ...
    target_url = f"{target_base}/graph/{path}"
