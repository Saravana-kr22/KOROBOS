...
Health route proxy — forwards /api/v1/health/* to the health-service.
"""
...
@router.api_route(
    ...
    operation_id="proxy_health",
)
async def proxy_health(request: Request, path: str):
    """Forward all /api/v1/health/* requests to the health-service."""
    registry = ServiceRegistry()
    target_base = registry.get_service_url("health")

    if not target_base:
        return JSONResponse(
            status_code=503,
            content={"status": "error", "error": {"code": "SERVICE_UNAVAILABLE", "message": "Health service not configured"}},
        )
